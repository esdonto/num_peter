...
def fazPredicao(ndig_treino=100, p=5):
    ...
    #Parte 2
    bateTeste = np.matrix(np.genfromtxt("dados_mnist/test_images.txt", usecols=range(10000))) / 255
    print(bateTeste.shape)
    Hs = dez * [0]
    normas = np.matrix([[0 for i in range(bateTeste.shape[1])] for j in range(dez)]) #10 linhas, 10000 colunas
    for i in range(dez):
        Hs[i] = resolveSimult(Ws[i], bateTeste)
        normas[i,:] = np.sqrt(np.square(bateTeste-Ws[i]*Hs[i]).sum(0))
    ...
    for i in range(10000):
        ...
        for j in range(1, dez):
            if maxNorma < normas[j,i]:
                predicao[i] = j
                maxNorma[i] = normas[j,i]
    np.savetxt("dados_gerados/predicao/predicao-n{}p{}.txt".format(ndig_treino,p), normas, delimiter=",")
